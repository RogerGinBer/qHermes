...
file <- "/media/hermes/Toshiba HDD/HermesResults/Ecoli/MS1_positive/UNL_Ecoli_MS1_pos_70-1000_01.mzML"
...
for (j in 1:nrow(roi)) {
    if(j%%100 == 0){print(j)}
    cur <- roi[j,]
    rt1 <- cur$rtmin; rt2 <- cur$rtmax
    mz1 <- cur$mzmin; mz2 <- cur$mzmax
...
pkscans %>% group_by(xcms_id) %>% summarise(n()) %>% .[,2] %>% unlist %>% unname %>% sort %>% plot

ecoliPos <- readRDS("/media/hermes/Toshiba HDD/HermesResults/Ecoli/Hermes_objects/PosResults.rds")
...
intersection <- dplyr::intersect(res$pkscans[,1:3],
...
library(qHermes)
library(RHermes)
ecoliPos <- readRDS("~/Datasets/HermesResults/Ecoli/Hermes_objects/PosResults.rds")
fil <- list.files("~/Datasets/HermesResults/Ecoli/MS1_positive", "mzML", full.names = T)[c(6,9,10,3,4)]
...
```{r}
XCMSnExp <- findChromPeaks(ms, CentWaveParam(5, c(5,25)))
res <- matchSOIToPeaks(XCMSnExp, ecoliPos, 4)

ft <- xcms::groupChromPeaks(XCMSnExp,
                            PeakDensityParam(
                              c("B", "B", "S", "S", "L"),
                              binSize = 0.05,
                              bw = 20,
...
q <-xcms::quantify(ft, "maxint")
q <- q@assays@data@listData[["raw"]]
q[is.na(q)] <- 0

fc <- log10((q[,2] + 1) / (q[,1] + 1))
hist(fc, breaks = 100, main = "Sample vs Blank")
fc <- log10((q[,3] + 1) / (q[,2] + 1))
hist(fc, breaks = 100, main = "Sample vs Sample-Replicate")
fc <- log10((q[,4] + 1) / (q[,2] + 1))
hist(fc, breaks = 100, main = "Labelled vs Sample")

copy <- XCMSnExp #We save a copy, since we modify the object

#### ChromPeaks that match a SOI ####

good_pks_id <- row.names(res$newpks)
chromPeaks(XCMSnExp) <- XCMSnExp@msFeatureData$chromPeaks[
   row.names(XCMSnExp@msFeatureData$chromPeaks) %in% good_pks_id, 
]
chromPeakData(XCMSnExp) <- chromPeakData(XCMSnExp)[
   row.names(chromPeakData(XCMSnExp)) %in% good_pks_id, 
]                                         
...
fc <- log10((q[,2] + 1) / (q[,1] + 1))
hist(fc, breaks = 100, main = "Match: Sample vs Blank")
fc <- log10((q[,3] + 1) / (q[,2] + 1))
hist(fc, breaks = 100, main = "Match: Sample vs Sample-Replicate")
fc <- log10((q[,4] + 1) / (q[,2] + 1))
hist(fc, breaks = 100, main = "Match: Labelled vs Sample")


XCMSnExp <- copy

#### ChromPeaks that don't match a SOI ####
...
fc_df <- data.frame(
  blank_sample = log10((q[,2] + 1) / (q[,1] + 1)),
  sample_rep = log10((q[,3] + 1) / (q[,2] + 1)),
  labelled_sample = log10((q[,4] + 1) / (q[,2] + 1))
)
fc <- log10((q[,2] + 1) / (q[,1] + 1))
hist(fc, breaks = 100, main = "No match: Sample vs Blank")
fc <- log10((q[,3] + 1) / (q[,2] + 1))
hist(fc, breaks = 100, main = "No match: Sample vs Sample-Replicate")
fc <- log10((q[,4] + 1) / (q[,2] + 1))
hist(fc, breaks = 100, main = "No match: Labelled vs Sample")
plot(fc_df)
```

```{r}
candidates <- which(fc_df$blank_sample > 2 & abs(fc_df$sample_rep) < 2)
cand_ft <- as.data.frame(featureDefinitions(ft)[candidates,])
table(cand_ft$S)
hist(cand_ft$mzmed, breaks = 100)
hist(cand_ft$rtmed, breaks = 100)
hist(cand_ft$npeaks, breaks = 100)
```
